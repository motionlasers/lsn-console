...
export default function Downloads() {
  ...
  const profileChannelMutation = useMutation({
    mutationFn: async (action: 'check' | 'activate' | 'rollback') => {
      ...
      activityApi.recordEvent({
        ...
        targetId: action,
        targetLabel: action.toUpperCase(),
        detail: {
          operation: action,
          versionNumber: state.active?.profileVersion,
          digestPrefix: state.active?.digest?.slice(0, 12),
          result: 'success'
        }
      });
    ...
    onError: (err, action) => {
      activityApi.recordEvent({
        ...
        targetId: action,
        targetLabel: action.toUpperCase(),
        detail: { operation: action, result: 'failure' }
      });
    }
  });
  ...
  const handleZipDownload = async () => {
    ...
    try {
      ...
      activityApi.recordEvent({
        eventName: 'DESKTOP_ACTION',
        clientEventId: generateEventId(),
        targetType: 'PACKAGE_GENERATION',
        targetId: 'zip',
        targetLabel: 'GENERATE_ZIP',
        detail: { operation: 'generate_zip', versionId: selectedVersion.id, versionNumber: selectedVersion.versionNumber, digestPrefix: selectedVersion.digest.slice(0, 12), result: 'success' }
      });
      ...
      setZipState('error');
      const errorMessage = err instanceof Error ? err.message : 'Unknown error generating package.';
      ...
      activityApi.recordEvent({
        ...
        targetId: 'zip',
        targetLabel: 'GENERATE_ZIP',
        detail: { operation: 'generate_zip', result: 'failure' }
      });
    }
  };
    ...
    try {
      ...
      if (content) {
        ...
        activityApi.recordEvent({
          ...
          targetType: 'PACKAGE_GENERATION',
          targetId: filename,
          targetLabel: `GENERATE_FILE_${filename}`,
          detail: { operation: 'generate_file', versionId: selectedVersion.id, fileName: filename, result: 'success' }
        });
      } else {
        throw new Error(`${filename} was not present in the generated package.`);
      }
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : `Unable to generate ${filename}.`;
      ...
      activityApi.recordEvent({
        ...
        targetId: filename,
        targetLabel: `GENERATE_FILE_${filename}`,
        detail: { operation: 'generate_file', fileName: filename, result: 'failure' }
      });
    } finally {
  ...
  const handleTrackedDownload = async (e: React.MouseEvent<HTMLAnchorElement>, asset: string, url: string, filename: string) => {
    if (isDesktop) {
      e.preventDefault();
      const recorded = await activityApi.recordEvent({
        eventName: 'DESKTOP_ACTION',
        clientEventId: generateEventId(),
        targetType: 'DOWNLOAD',
        targetId: asset,
        targetLabel: `DOWNLOAD_${asset.toUpperCase()}`,
        detail: { operation: 'download', asset, result: 'success' }
      });
      if (!recorded) {
        setItemError('The download was not opened because its audit event could not be recorded.');
        return;
      }
                ...
                {releaseAssetUrl(WINDOWS_ARTIFACTS.installer) ? (
                  <div className="flex flex-col gap-2">
                    <Button asChild className="w-full font-mono text-xs h-10 tracking-wider" data-testid="button-download-installer">
                      <a href={getDownloadUrl('installer', WINDOWS_ARTIFACTS.installer)!} onClick={(e) => handleTrackedDownload(e, 'installer', getDownloadUrl('installer', WINDOWS_ARTIFACTS.installer)!, WINDOWS_ARTIFACTS.installer)} download>
                    ...
                    <Button asChild variant="outline" className="w-full font-mono text-xs h-9 tracking-wider" data-testid="button-download-portable">
                      <a href={getDownloadUrl('portable', WINDOWS_ARTIFACTS.portable)!} onClick={(e) => handleTrackedDownload(e, 'portable', getDownloadUrl('portable', WINDOWS_ARTIFACTS.portable)!, WINDOWS_ARTIFACTS.portable)} download>
                        <Download className="w-3 h-3 mr-2" /> PORTABLE ZIP (OPTIONAL)
                      </a>
                    </Button>
                    <a
                      href={getDownloadUrl('checksums', WINDOWS_ARTIFACTS.checksumManifest)!}
                      onClick={(e) => handleTrackedDownload(e, 'checksums', getDownloadUrl('checksums', WINDOWS_ARTIFACTS.checksumManifest)!, WINDOWS_ARTIFACTS.checksumManifest)}
