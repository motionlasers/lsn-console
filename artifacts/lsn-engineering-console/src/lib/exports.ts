...
import { getReleaseExportMetadata } from './release';
...
export function downloadBlob(blob: Blob, filename: string) {
  ...
  if (bridge) {
    // Desktop shell: use the native save dialog through the narrow preload
    // bridge. A user-cancelled dialog ({ saved: false } without error) is
    // intentional and ends the export; a bridge failure or write error falls
    // back to the browser anchor flow so the export is never silently lost.
    void blob.arrayBuffer()
      .then(buffer => bridge.saveFile(filename, new Uint8Array(buffer)))
      .then(result => {
        if (!result.saved && result.error) browserDownloadBlob(blob, filename);
      })
      .catch(() => browserDownloadBlob(blob, filename));
    return;
  }
  ...
  return `
    ...
    <body>
      ...
      <div class="metadata">
        ...
        <p><strong>Generated by:</strong> LSN Engineering Console ${release.consoleRelease}</p>
        <p><strong>Protocol:</strong> ${release.protocolVersion} · <strong>Device Profile:</strong> ${release.deviceProfileVersion} · <strong>Interface Package:</strong> ${release.firmwareInterfacePackage}</p>
      </div>
      <table>
        <tr>
          <th>Test Name</th>
